module Make (Conf : Service.CONF) = struct
  ...
  module Service = Service.Make (Conf)
    (struct let name = "sqs" and host = endpoint Conf.region end)
  ...
  let create_queue
    ...
    let%lwt res, _ = Service.request ~meth:`POST ~uri:"/" ~query:parameters () in
  ...
  let get_queue_attributes ~attributes ~queue_url () =
    ...
    in
    let%lwt res, _ =
      Service.request ~meth:`POST ~uri:"/" ~query:parameters () in
    Lwt.return
