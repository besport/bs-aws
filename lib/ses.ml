module Make (Conf : Service.CONF) = struct
  ...
  let message_tag prefix (name, value) rem =
    string (prefix ^ ".Name") name @@ string (prefix ^ ".Value") value @@ rem

  let action act = ["Version", "2010-12-01"; "Action", act]

  let send_email ?configuration_set_name ~destination ~message
      ?(reply_to_addresses = []) ?return_path ?return_path_arn ~source
      ?source_arn ?(tags = []) ()
    =
    let parameters =
      action "SendEmail"
