...
module Make (Conf : Service.CONF) = struct
  ...
  let bucket_url bucket =
    ...
    if Conf.secure
    then Format.sprintf "https://%s.s3-%s.amazonaws.com/" bucket region
    else Format.sprintf "http://%s.s3-%s.amazonaws.com/" bucket region
  ...
  let form ~bucket ~expiration ~key ?content_length_range
      ?success_action_redirect ?success_action_status ?(other_fields = []) ()
    =
    let date = Unix.gettimeofday () in
    let expiration =
      match expiration with `Delay d -> date +. d | `Date d -> d
    in
    ...
    let cond c =
      match c with `Eq _ as c -> c | `Prefix (p, _) -> `Prefix p
    in
    ...
    let conditions =
      ...
      | None -> conditions
      | Some url ->
          condition "success_action_redirect" (cond url) :: conditions
    in
    let conditions =
    ...
    let conditions =
      ...
      | Some (min, max) ->
          `List
            ...
            ; `String (string_of_int min)
            ; `String (string_of_int max) ]
          :: conditions
      | None -> conditions
    in
    let conditions =
      List.fold_left
        (fun rem (f, v) -> condition f v :: rem)
        conditions other_fields
    in
    let policy =
      `Assoc
        [ "expiration", `String (Base.to_ISO8601 ~extended:true expiration)
  ...
  let object_url ~bucket ~expiration ~key ?response_content_type
      ?response_content_language ?response_expires ?response_cache_control
    ...
    let {Base.uri; query} =
      ...
      |> Signature.sign_request_using_query_parameters credentials
           ~service:"s3" Conf.region ~expiration ~unsigned_payload:true
    in
    Lwt.return
  ...
  let request ~meth ?host ~uri ?query ?(headers = []) ?(payload = "")
      ?(hash = hash payload) ()
    =
    let%lwt res, _ =
      Service.request ~meth ?host ~uri ?query ~headers:(hash :: headers)
        ~payload ()
    in
    Lwt.return res
  ...
  module Bucket = struct
    ...
    let list ?prefix bucket =
      let query = Base.Param.(["list-type", "2"] |> string "prefix" prefix) in
      ...
      let contents =
        List.map
          (fun c -> {key = data_to_string @@ member "Key" c})
          (members "Contents" r)
      in
  ...
  module Object = struct
    let delete ~bucket object_name =
      let%lwt _ =
        request ~meth:`DELETE ~host:(bucket_host bucket)
          ~uri:("/" ^ object_name) ()
      in
      Lwt.return_unit
    ...
    let put ~bucket object_name data =
      let%lwt _ =
        request ~meth:`PUT ~host:(bucket_host bucket) ~uri:("/" ^ object_name)
          ~payload:data ()
      in
      Lwt.return_unit
    ...
    let copy ~src_bucket src_object ~dst_bucket dst_object =
      let%lwt _ =
        request ~meth:`PUT ~host:(bucket_host dst_bucket)
    ...
    let head ~bucket key =
      request ~meth:`HEAD ~uri:("/" ^ bucket ^ "/" ^ key) ()
