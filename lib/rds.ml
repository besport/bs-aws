...
let parse_status_info i =
  ...
  ; message =
      (if has_member "Message" i
      then Some (data_to_string (member "Message" i))
      else None) }
...
module Make (Conf : Service.CONF) = struct
  ...
  let describe_db_instances ?db_instance_identifier () =
    ...
    let l =
      ...
      |> member "DBInstances" |> members "DBInstance"
      |> List.map (fun i ->
             { db_instance_identifier =
                 data_to_string (member "DBInstanceIdentifier" i)
             ; endpoint =
                 (if has_member "Endpoint" i
                 then Some (parse_endpoint @@ member "Endpoint" i)
                 else None)
             ; db_instance_status =
                 data_to_string (member "DBInstanceStatus" i)
             ; status_infos =
                 (if has_member "StatusInfos" i
                 then
                   List.map parse_status_info
                     (members "DBInstanceStatusInfo" (member "StatusInfos" i))
                 else [])
