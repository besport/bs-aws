module Make (Conf : Service.CONF) = struct
  ...
  module Service =
    Service.Make
      (Conf)
      (struct
        let name = "mediaconvert"
        and host = endpoint Conf.region
      end)

  let describe_endpoints ?next_token ?max_results ?mode () =
    ...
    let query =
      ...
      |> custom "mode"
           (fun m ->
             match m with `DEFAULT -> "DEFAULT" | `GET_ONLY -> "GET_ONLY")
           mode
    in
    ...
    let endpoints =
      ...
      if e = `Null
      then []
      else List.map (fun u -> to_string @@ member "url" u) (to_list e)
    in
  ...
  let job_description res =
    ...
    let job_percent_complete =
      to_int_option @@ member "jobPercentComplete" job
    in
    Lwt.return
      { id
      ; status
  ...
  let create_job ~endpoint ?client_request_token ?job_template ?queue ~role
      ~settings ?user_metadata ?billing_tags_source ?acceleration_settings
    ...
    let%lwt res, _ =
      Service.request ~host:(endpoint_host endpoint) ~meth:`POST ~uri ~payload
        ()
    in
    job_description res

  let get_job ~endpoint ~id () =
    let uri = Printf.sprintf "/2017-08-29/jobs/" ^ id in
    let%lwt res, _ =
      Service.request ~meth:`GET ~host:(endpoint_host endpoint) ~uri ()
    in
    job_description res
end
