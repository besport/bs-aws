module Make (Conf : Service.CONF) = struct
  ...
  module Service =
    Service.Make
      ...
      (struct
        let name = "sns"
        and host = sns_endpoint Conf.region
      end)
  ...
  let subscribe ~endpoint ~protocol ~topic_arn ?(attributes = []) () =
    let query =
  ...
  let unsubscribe ~subscription_arn () =
    let query = init_params "Unsubscribe" "SubscriptionArn" subscription_arn in
    let%lwt _ = Service.request ~meth:`POST ~uri:"/" ~query () in
    Lwt.return_unit

  let set_subscription_attributes ~attribute ~subscription_arn () =
    let query =
      ( "AttributeName"
      ...
      :: ( "AttributeValue"
         , match attribute with
           | `Delivery_policy p -> Yojson.Safe.to_string p
           | `Raw_message_delivery b -> string_of_bool b )
      :: init_params "SetSubscriptionAttributes" "SubscriptionArn"
           subscription_arn
    in
    let%lwt _ = Service.request ~meth:`POST ~uri:"/" ~query () in
    Lwt.return_unit
